        ...
        fs.readdir(
          ...
          (err, files) => {
            ...
            var finalname =
              arr.slice(0, -1).join(".") +
              "_" +
              files.filter((f) => f.isFile()).length +
              "." +
...
function upload_form() {
  var storage = multer.diskStorage({
    destination: async function (req, file, cb) {
      try {
        var result = await set_dest(file.originalname, get_type(req.url));
        cb(null, result);
      } catch (err) {
        cb(err, null);
      }
    ...
    filename: async function (req, file, cb) {
      try {
        var result = await set_filename(file.originalname, get_type(req.url));
        cb(null, result);
      } catch (err) {
        cb(err, null);
      }
...
function db_insert(file, meta, return_id = false) {
  return new Promise((resolve, reject) => {
    ...
    if (typeof meta.tendencies !== "undefined") {
      row.tendencies = meta.tendencies;
    }
    if (typeof meta.notes !== "undefined") {
    ...
    knex(table)
      .insert(row)
      .then((id) => {
        resolve(return_id ? id[0].toString() : "uploaded asset!");
      })
      .catch((err) => {
        reject(err);
      });
