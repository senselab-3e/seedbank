...
function draw() {
  $.fillStyle = "#ffffff"; //"hsla(252, 95%, 85%, 1)";
...
var Ball = function () {
  ...
  this.col =
    ...
    "," +
    "0.3" +
    ")"; //introduced alpha level through 0.3
  this.g = 10; //---->gravity force
  this.vx = 0;
  this.vy = 0;
...
var Line = function (x, y) {
  ...
  this.checked = function (it) {
    return (
      Math.sqrt(Math.pow(it.ex - it.sx, 2) + Math.pow(it.ey - it.sy, 2)) >
      Line.min
    );
  };
};
...
var Plank = function () {
  ...
  this.start = function (it, x, y) {
    if (it.curr == null) it.curr = new Line(x, y);
  };
  this.fin = function (it, x, y) {
    it.curr.ex = x;
    it.curr.ey = y;
    if (it.curr.checked(it.curr)) {
      if (it.planks.length >= Plank.max) it.planks.shift();
