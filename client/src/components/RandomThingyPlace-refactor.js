...
export class RandomThingyPlaceref extends Component {
  componentDidMount() {
    createLinks();
  }
  render() {
    ...
    const dotRandPos = () => {
      dotpatches.forEach(function(patch) {
        ...
        patch.addEventListener("mouseover", event => {
          patch.classList.add(chooseElement(aniClassOpt));
          patch.classList.add("animated");
        });
        patch.addEventListener("mouseout", event => {
          patch.classList.remove("animated");
          patch.classList.remove("rubberBand", "shake", "swing", "pulse");
        });
      });
    };
...
export default RandomThingyPlaceref;
