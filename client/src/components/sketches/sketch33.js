...
export default function sketch33(p) {
    ...
    let colorOptions = ['yellow', 'orange', 'lightblue', 'lightgreen', 'magenta', 'deeppink', 'purple']
    ...
    p.setup = function () {
        ...
        p.background('grey');
        planet = new Orbit(0, 0, 14, 'teal', 12)
    ...
    p.mousePressed = function () {
        ...
        if (drawings.length < 20) {
            planet = new Orbit(p.mouseX, p.mouseY, p.floor(p.random(15, 32)), p.random(colorOptions), p.floor(p.random(3, 32)))
            drawings.push(planet)
        } else {
            drawings.splice(0, 1);
            planet = new Orbit(p.mouseX, p.mouseY, p.floor(p.random(15, 32)), p.random(colorOptions), p.floor(p.random(3, 32)))
