...
export default function sketch35(p) {
    ...
    p.setup = function () {
        ...
        repeller = new Repel(width / 2, height / 2, 100);
        //
        p.background(0);
    ...
    p.draw = function () {
        ...
        for (let mover of movers) {
            ...
            if (p.mouseIsPressed) {
                ...
                repeller.pos.x = p.mouseX;
                repeller.pos.y = p.mouseY;
                p.clear();
                p.background(0);
    ...
    class Attractor {
        ...
        attract(mover) {
            let force = p5.Vector.sub(this.pos, mover.pos); //calculate the mover and the attractor
            ...
            let distanceSq = p.constrain(force.magSq(), 100, 1000); //calculate alll the elements to calculate the magnitude of that vector.
            let G = 2. //-0.005;
            let strength = (G * (this.mass * mover.mass)) / distanceSq; /// put all of that calculation into a variable called strength
            force.setMag(strength);
            // and then set the magnitude of that vector to stength
            mover.applyForce(force); /// and then apply that force --- where the applyForce function add the xy amounts to increase the movements
        }
